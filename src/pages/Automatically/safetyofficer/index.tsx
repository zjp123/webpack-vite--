...
import { saveExamineeCheckApi, safAssignScrollList } from "@/api/automatically"
import "./index.less"
...
const Safetyofficer = () => {
  ...
  useEffect(()=>{
    const data = {
      idCard: idCardInfo?.certNumber,/* 安全员*/ userType: 2, imgUrl
    }
    if (imgUrl) {
      // 如果拍照成功, 保存提交核验信息
  ...
  const getListData = (data) => {
    safAssignScrollList(data).then((res) => {
      const list = res?.data?.map((item, index) => ({ id: index, ...item })) || []
      setTableList(list)
    })
  }
  ...
  const saveExamineeCheck = (data)=>{
    ...
    saveExamineeCheckApi({...data, macCode: "DN-9100_42AD54E06DCD"}).then((res) => {
      const { data={}} = res
      if (res?.code === 0) {
...
export default connect(({}) => ({}))(Safetyofficer)
