    ...
    <Modal
      ...
      onOk={() => {
        form.validateFields().then(async (res) => {
          ...
          try {
            await dispatch({
              type: 'recording/addModification',
              payload: {
                ...res,
                id: personId
              }
    ...
    >
      <Form layout="horizontal" form={form} colon={false} autoComplete="off">
        <Row style={{padding: '8px 40px', justifyContent: 'space-between'}}>
          <Col span={12}>
            <Form.Item {...FORMITEM_LAYOUT} rules={[{pattern: TEL_REGEXP, message: '请输入正确的联系电话'}]} name="contactPhone"
                       label="联系电话">
              <Input placeholder="请输入" maxLength={11}/>
            </Form.Item>
          </Col>
          <Col span={12}>
            <Form.Item rules={[{pattern: EMAIL_REGEXP, message: '请输入正确的邮箱格式'}]} {...FORMITEM_LAYOUT} name="eMail"
                       label="电子邮箱">
