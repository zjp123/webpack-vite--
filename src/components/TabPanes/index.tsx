...
const TabPanes: FC<Props> = props => {
  ...
  //权限路由第一个
  let homePanela = globalRoutes[0].component ? globalRoutes[0] : globalRoutes[0].routes[0]
  homePanela.content = homePanela.component
  homePanela.closable = false
  let homePanel = [homePanela]
  const homePanelKey = homePanel[0].key
  ...
  const resetTabs = useCallback((): void => {
    const initPanes = curTab.reduce((prev: CommonObjectType[], next: string) => {
      const { title, tabKey, component: Content } = getKeyName(next, globalRoutes)
      return [
        ...prev,
        {
          title,
          key: tabKey,
          content: Content,
          closable: tabKey !== resetTabs,
          path: next
        }
      ]
    }, [])
    const { tabKey } = getKeyName(pathname, globalRoutes)
    setPanes(initPanes)
    setActiveKey(tabKey)
  }, [curTab, pathname])
  ...
  const remove = (targetKey: string): void => {
    ...
    const { tabKey } = getKeyName(nextPath, globalRoutes)
  ...
  const removeAll = async (isCloseAll?: boolean) => {
    ...
    const nowPanes = key !== homePanelKey && !isCloseAll ? [...homePanel, selectedPanel] : homePanel
    setPanes(nowPanes)
    setActiveKey(isCloseAll ? homePanelKey : key)
    storeTabs(nowPanes)
  }
  ...
  return (
    <div>
      <Tabs activeKey={activeKey} className={style.tabs} defaultActiveKey={defaultActiveKey} hideAdd onChange={onChange} onEdit={onEdit} onTabClick={onTabClick} type="editable-card">
        {panes.map((pane: CommonObjectType) => (
          ...
          >
            {reloadPath !== pane.path && pane.content ? (
