...
const openData = {
  ...
  fingerFap: '1', // 指纹   0：只签字   1:  签字加指（若设备有指纹模块）  2： 只指纹（若设备有指纹模块）
 ...
 * @constructor
 */
const HWOptimizedSignature: FC<Props> = props => {
  const {
    initialImmediately = true,
    outerWidth=460, width = 300, height = 200, prompt, getSignedUrl, getSignedBase64Result, delay = 0, styleIndex = 1,
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.msgID === '0') {
          ...
          openNotification({message: '初始化成功', duration: 2.5, description: '设备已经打开, 请开始签字... ...'}, 'success')
...
export default HWOptimizedSignature
